...
import { mercatorZfromAltitude } from './mercator-coordinates';
...
interface MapTransform {
  ...
  worldSize: number;
  elevation: number;
  _camera: { position: number[] };
  maxPitch: number;
  fov: number;
  _fov: number;
  centerOffset: { y: number };
}
...
export default class CameraSync {
  ...
  update() {
    ...
    const fovAboveCenter =
      fovRad * (0.5 + this.map.transform.centerOffset.y / height);
    ...
    if (elevation)
      cameraWorldMatrix.elements[14] = _camera.position[2] * worldSize;
